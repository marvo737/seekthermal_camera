...
SeekThermalRos::SeekThermalRos(const std::string nodeName, ros::NodeHandle& nh, ros::NodeHandle& nh_priv)
  : nodeName(nodeName), nh(nh), nh_priv(nh_priv)
{
  ...
  if (ffc_filename != "")
  {
    ...
    ffc_image = cv::imread(ffc_filename, cv::IMREAD_UNCHANGED);
    if (ffc_image.empty())
    {
